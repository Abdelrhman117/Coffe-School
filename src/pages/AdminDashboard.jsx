...
const STATUS_LABELS = { pending: 'Pending', confirmed: 'Confirmed', cancelled: 'Cancelled', delivered: 'Delivered' }
...
export default function AdminDashboard({ onClose }) {
    ...
    return (
      ...
      >
        <div className="glass-dark rounded-2xl p-10 text-center space-y-4 border border-red-500/20 max-w-sm">
          ...
          <p className="text-[#c5a059]/50 text-sm">This page is for admins only</p>
          <button onClick={onClose} className="btn-gold px-8 py-2.5 rounded-xl text-sm">Go Back</button>
        </div>
      </motion.div>
    )
  }
  ...
  const handleSave = async () => {
    if (!form.name.trim() || !form.price) { addToast('Name and price are required', 'warning'); return }
    const price = parseFloat(form.price)
    if (isNaN(price) || price <= 0) { addToast('Price must be a positive number', 'warning'); return }
    setSaving(true)
    const data = {
      ...form, price,
  ...
  return (
    ...
    >
      {/* ── SIDEBAR ───────────────────────────────────────────────────────────── */}
      <motion.aside
        className="hidden md:flex flex-col w-60 shrink-0 border-r border-[#c5a059]/10 bg-[#080604]"
      ...
      >
        ...
        <div className="flex items-center gap-2.5 px-5 py-6 border-b border-[#c5a059]/10">
          ...
          <div>
            <p className="text-[#c5a059] font-black text-sm leading-none">Admin Panel</p>
        ...
        <nav className="flex-1 p-3 space-y-1">
          {NAV_ITEMS.map(({ id, label, icon: Icon }) => (
            ...
            >
              ...
              {id === 'products' && (
                <span className={`relative z-10 ml-auto text-[10px] font-black px-1.5 rounded-full ${tab === id ? 'bg-[#050302]/20 text-[#050302]' : 'bg-[#c5a059]/15 text-[#c5a059]/60'}`}>
                  {products.length}
                </span>
              )}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="md:hidden flex items-center justify-between px-4 py-4 border-b border-[#c5a059]/10 shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[#c5a059] font-black text-sm">Admin Panel</span>
          </div>
          ...
          <AnimatePresence mode="wait">
            {tab === 'products' && (
              ...
              >
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h2 className="text-xl font-black text-[#e5d5b0]">Products & Courses</h2>
                    <p className="text-xs text-[#c5a059]/40 mt-0.5">{products.length} items</p>
                  </div>
                  <motion.button
                    onClick={openAdd}
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1.5">
                              ...
                              >
                                {p.type === 'course' ? 'Course' : 'Product'}
                              </span>
                              {p.badge && <span className="text-[9px] text-[#c5a059]/40 truncate">{p.badge}</span>}
                            </div>
                            <h3 className="font-bold text-[#e5d5b0] text-sm leading-snug">{p.nameEn || p.name}</h3>
                            ...
                            <p className="text-[#c5a059] font-black text-base mt-2.5">
                              {Number(p.price).toLocaleString('en-EG')}
                              <span className="text-[10px] font-normal ml-1">{p.currency}</span>
                            </p>
                          </div>
            ...
            {tab === 'orders' && (
              ...
              >
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h2 className="text-xl font-black text-[#e5d5b0]">Incoming Orders</h2>
                    <p className="text-xs text-[#c5a059]/40 mt-0.5">{orders.length} orders</p>
                  </div>
                  <motion.button
                    onClick={fetchOrders}
                ...
                {ordersLoading ? (
                  ...
                  >
                    ...
                    <p className="text-xs mt-1 opacity-60">Orders will appear here once placed</p>
                  </motion.div>
                ) : (
                        ...
                        >
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex-1 min-w-0">
                              ...
                              {order.contactPhone && (
                                <p className="text-xs text-[#c5a059]/50 mt-0.5">{order.contactPhone}</p>
                              )}
                              <p className="text-xs text-[#c5a059]/40 mt-1">
                                {order.items?.length} items ·{' '}
                                {order.items?.map((i) => i.nameEn || i.name).join(', ').slice(0, 55)}
                                {order.items?.map((i) => i.nameEn || i.name).join(', ').length > 55 ? '...' : ''}
                              </p>
                            </div>
                            <div className="text-right shrink-0">
                              ...
                              {order.createdAt?.toDate && (
                                <p className="text-[10px] text-[#c5a059]/30 mt-0.5">
          ...
          <>
            ...
            >
              ...
              <div className="flex items-center justify-between px-6 py-5 border-b border-[#c5a059]/15 shrink-0">
                <h3 className="font-bold text-[#c5a059]">{editId ? 'Edit Product' : 'Add New Product'}</h3>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <FField label="Arabic Name *">
                    <input className="w-full input-gold rounded-xl px-3 py-2.5 text-sm" value={form.name} onChange={setField('name')} placeholder="Barista Course..." />
                  </FField>
                  <FField label="English Name">
                    <input className="w-full input-gold rounded-xl px-3 py-2.5 text-sm" value={form.nameEn} onChange={setField('nameEn')} placeholder="Barista Course" />
                  </FField>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <FField label="Type">
                    <select className="w-full input-gold rounded-xl px-3 py-2.5 text-sm" value={form.type} onChange={setField('type')}>
                      {TYPES.map((t) => <option key={t} value={t}>{t === 'course' ? 'Course' : 'Product'}</option>)}
                    </select>
                  </FField>
                  <FField label="Price (EGP) *">
                    <input type="number" className="w-full input-gold rounded-xl px-3 py-2.5 text-sm" value={form.price} onChange={setField('price')} placeholder="3000" min="0" />
                  </FField>
                  <FField label="3D Model">
                ...
                <FField label="Description">
                  <textarea className="w-full input-gold rounded-xl px-3 py-2.5 text-sm resize-none" rows={3} value={form.description} onChange={setField('description')} placeholder="Detailed description..." />
                </FField>

                <div className="grid grid-cols-2 gap-3">
                  <FField label="Variant">
                    <input className="w-full input-gold rounded-xl px-3 py-2.5 text-sm" value={form.variant} onChange={setField('variant')} placeholder="250g" />
                  </FField>
                  <FField label="Partner">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <FField label="Badge">
                    <input className="w-full input-gold rounded-xl px-3 py-2.5 text-sm" value={form.badge} onChange={setField('badge')} placeholder="Best Seller" />
                  </FField>
                  <FField label="Light Color">
                    <div className="flex items-center gap-2">
                      ...
                      <input className="flex-1 input-gold rounded-xl px-3 py-2.5 text-sm font-mono" value={form.lightColor} onChange={setField('lightColor')} />
                    </div>
                  </FField>
                </div>

                <FField label="Features (comma separated)">
                  <input className="w-full input-gold rounded-xl px-3 py-2.5 text-sm" value={form.features} onChange={setField('features')} placeholder="Certified, Hands-on training, 5 days" />
                </FField>

                <FField label="Image URL">
                  <input className="w-full input-gold rounded-xl px-3 py-2.5 text-sm" value={form.image} onChange={setField('image')} placeholder="/textures/product.jpg" />
                </FField>
              </div>
        ...
        {deleteConfirm && (
          ...
          >
            ...
            >
              ...
              <p className="text-[#e5d5b0] font-bold text-lg">Are you sure?</p>
              <p className="text-[#c5a059]/50 text-sm">This action cannot be undone</p>
