...
export default function Navbar() {
  ...
  return (
    <motion.nav
      className="fixed top-0 right-0 left-0 z-[8000] glass-dark"
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6 text-sm text-[#c5a059]/70 font-medium">
          {[['#hero', 'الرئيسية'], ['#products', 'المنتجات'], ['#courses', 'الدورات'], ['#contact', 'اتصل بنا']].map(([href, label]) => (
            <a key={href} href={href} className="hover:text-[#c5a059] transition-colors py-1 relative group">
              {label}
              <span className="absolute bottom-0 right-0 w-0 h-px bg-[#c5a059] group-hover:w-full transition-all duration-300" />
            </a>
          ))}
        </div>
            ...
            <AnimatePresence>
              {cartCount > 0 && (
                <motion.span
                  ...
                  animate={{ scale: 1 }}
                  exit={{ scale: 0 }}
                  className="absolute -top-1 -left-1 bg-[#c5a059] text-[#050302] text-[10px] font-black rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1"
                >
                  {cartCount}
                </motion.span>
              )}
              ...
              <AnimatePresence>
                {menuOpen && (
                  <motion.div
                    ...
                    className="absolute left-0 top-full mt-2 w-44 glass-dark rounded-xl overflow-hidden shadow-xl py-1 border border-[#c5a059]/15"
                  >
                    {isAdmin && (
                      <DropdownItem icon={<Settings size={15} />} onClick={() => { toggleAdminPanel(); setMenuOpen(false) }}>
                        لوحة الإدارة
                      </DropdownItem>
                    )}
                    <DropdownItem icon={<LogOut size={15} />} onClick={handleSignOut} danger>
                      تسجيل الخروج
                    </DropdownItem>
                  </motion.div>
                )}
