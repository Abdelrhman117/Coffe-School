...
export default function AuthModal() {
  ...
  const handleDemoLogin = () => {
    ...
    setUser(fakeUser)
    addToast('Signed in (demo mode)', 'info')
    closeAuthModal()
  }
  ...
  const handleSendOtp = async () => {
    if (DEMO_MODE) {
      setOtpSent(true)
      addToast('OTP sent (demo mode)', 'info')
      return
    }
    if (!form.phone) { addToast('Please enter your phone number', 'warning'); return }
    setLoading(true)
    try {
  ...
  const handleVerifyOtp = async () => {
    if (DEMO_MODE) { handleDemoLogin(); return }
    if (!form.otp) { addToast('Please enter the verification code', 'warning'); return }
    setLoading(true)
    try {
  ...
  const handleApple = async () => {
    ...
    try {
      ...
      await ensureUserDoc(cred.user)
      setUser(cred.user)
      addToast('Signed in with Apple successfully', 'success')
      closeAuthModal()
    } catch (err) {
      if (err.code !== 'auth/popup-closed-by-user') {
        addToast(mapError(err.code), 'error')
      ...
      <div className="glass-dark rounded-2xl w-full max-w-[420px] overflow-hidden shadow-2xl">
        ...
        <div className="p-6 space-y-5">
            ...
            <form onSubmit={handleEmailSubmit} className="space-y-3">
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-[#c5a059]/50 hover:text-[#c5a059]"
                >
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-[#c5a059]/40 text-xs">or</span>
            <div className="flex-1 h-px bg-[#c5a059]/15" />
          </div>
